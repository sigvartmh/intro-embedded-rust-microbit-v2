...
fn init_device(p: hal::pac::Peripherals) -> (Pwm<hal::pac::PWM0>, Timer<hal::pac::TIMER0>) {
    ...
    /* Colum 1 and 5 */
    p0.p0_28.into_push_pull_output(gpio::Level::Low).degrade();
    p0.p0_30.into_push_pull_output(gpio::Level::Low).degrade();

    pwm.set_output_pin(
        pwm::Channel::C0,
        p0.p0_20.into_push_pull_output(gpio::Level::High).degrade(),
    );
    pwm.set_output_pin(
        pwm::Channel::C1,
        p0.p0_21.into_push_pull_output(gpio::Level::High).degrade(),
    );
